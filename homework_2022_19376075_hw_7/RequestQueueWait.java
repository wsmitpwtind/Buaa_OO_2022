...
public class RequestQueueWait {
    ...
    public synchronized int getOuterTar(int nowFloor, int nowDirection, String type, int info) {
        if (requests.isEmpty()) {
            notifyAll();
            return -1;
        }
        ...
        if (Objects.equals(type, "building")) {
            ...
            flag = getFlag(nowDirection, nowFloor, info);
            if (flag == -1) {
                for (int i = 0; i < requests.size(); i++) {
                    int floor = requests.get(i).getFromBuilding() - 'A' + 1;
                    int nowFloorCode = 1 << (floor - 1);
                    boolean acceptAble = (nowFloorCode & info) != 0;
                    if (floor == nowFloor && acceptAble) {
                        flag = i;
                    }
                }
                for (int i = 0; i < requests.size(); i++) {
                    ...
                    int nowFloorCode = 1 << (floor - 1);
                    boolean acceptAble = (nowFloorCode & info) != 0;
                    if (floor == (nowFloor - 2 * nowDirection + 4) % 5 + 1 && acceptAble) {
                        flag = i;
                        break;
                    }
                    if (floor == (nowFloor - nowDirection + 4) % 5 + 1 && acceptAble) {
                        flag = i;
                    }
                }
            }
    ...
    public synchronized boolean isEmpty(int switchInfo) {
        if (switchInfo == 0) {
            notifyAll();
            return requests.isEmpty();
        } else {
            for (PersonRequest request : requests) {
                if (((1 << (request.getFromBuilding() - 'A')) & switchInfo) != 0)
                {
                    notifyAll();
                    return false;
                }
    ...
    private synchronized int getFlag(int nowDirection, int nowFloor, int info) {
        //此处未考虑能否停靠
        int flag = -1;
        if (nowDirection == 0) {
            for (int i = 0; i < requests.size(); i++) {
    ...
    public synchronized boolean isIn(ElevatorProcess elevatorProcess, String type) {
        ...
        if (Objects.equals(type, "building")) {
            ...
            for (PersonRequest request : requests) {
                ...
                int requestFloor = request.getFromBuilding() - 'A' + 1;
