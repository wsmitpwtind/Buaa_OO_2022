...
public class JvmHeap extends MyHeap<MyObject> {
    ...
    /*@ public normal_behavior
      ...
      @          (\forall int j; 0 <= j && j < objectId.size();
      @            objectId.get(j) != elements[i].getId()) ==>  (elements[i].isReferenced() == \old(elements[i].isReferenced())));
    ...
    public void removeUnreferenced() {
        ...
        for (int i = 1; i <= getSize();i++) {
            if (getElement(i).isReferenced()) {
                objects.add(getElement(i));
            }
        }
        clear();
